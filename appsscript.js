...
function doPost(e) {
  ...
  // ── Email notification ──
  try {
    const subject = '🍪 New Cookie Order — ' + (p.name || 'Unknown');
    const body = [
      'A new order was just submitted on the Sugar & Shai Cookies website!',
      '',
      'Name:         ' + (p.name        || '—'),
      'Email:        ' + (p.email       || '—'),
      'Contact:      ' + (p.contact     || '—'),
      ...
      'Notes:        ' + (p.notes       || '—'),
      '',
      'Open your Google Sheet to view all orders.',
    ].join('\n');
